...
// viewer (south). Room shells are 6 × 5 m. yaw 0 means the model faces +Z.
//
// Every placement is derived from the measured model footprints (see CREDITS/§P3
// notes), so clearances are real: e.g. the engineering desk is 1.17 × 0.69 m with
// its 0.78 m top at y = 0.768, and chair rows leave a 1.04 m centre aisle.
//
// P3-fix:
// - Zone A standing array reduced from 12 to 4 spots. 12 overflow people crammed
//   around a conference table looked wrong; 4 gives a natural "presenter + two
//   observers" look when occupancy exceeds the 8 seat count.
// - ceilingFan removed from Zone C furniture list. It is now rendered by FanMesh
//   in BuildingScene3D with an airflow_lps-driven rotation. Keeping it in both
//   lists would double-render the mesh and cause z-fighting.
...
// desk.glb = 0.734 × 0.384 × 0.392 units; these multipliers (on top of
// FURNITURE_SCALE) give a 1.17 × 0.77 × 0.69 m workstation.
...
// ── A — Conference: 2.30 × 1.21 m table, 8 chairs, 2 plants ─────────────────
...
export const ROOM_SCENE: Record<RoomId, RoomScene> = {
  A: {
    ...
    seats: A_CHAIRS,
    // Reduced from 12 to 4 overflow spots. A conference room looks correct
    // with a presenter at the end and two observers by the walls — not with
    // 12 people ring-fencing the table.
    standing: [
      { x: -2.3, z:  0,    ry:  HALF_PI },  // left-wall presenter
      { x:  2.3, z:  0,    ry: -HALF_PI },  // right-wall observer
      { x:  0,   z: -1.85, ry:  0        },  // near end, facing table
      { x:  0.8, z: -1.85, ry:  0        },  // near end, secondary
    ],
  },
  ...
  C: {
    furniture: [
      ...
      // ceilingFan intentionally omitted — rendered by FanMesh in BuildingScene3D.tsx
      // with an airflow_lps-driven rotation speed. Both would z-fight.
